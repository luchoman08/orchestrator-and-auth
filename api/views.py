...
def generate_assignment_with_attributes(request):
    remote_url = 'http://user-stories-assignment:3001/attributeassign/'
    print(request.data)
    response = requests.post(remote_url, json=request.data)
    return JsonResponse(json.loads(response.text), safe=False)


@api_view(['POST'])
@permission_classes((AllowAny, ))
def generarAsignacion(request):
    remote_url = 'http://user-stories-assignment:3001/uniquecostassign/'
    print(request.data)
    response = requests.post(remote_url, json=request.data)
    return JsonResponse(json.loads(response.text), safe=False)
...
def generate_pair_assignment(request):
    remote_url = 'http://user-stories-assignment:3001/pairassign/'
    print(request.data)
    response = requests.post(remote_url, json=request.data)
    return JsonResponse(json.loads(response.text), safe=False)
...
def general_project_management(req):
    ...
    remote_url = 'http://project-managers-interface:3000' + req.path.replace('/api/v1', '')
    response = {}
    if req.method == 'PATCH':
        print(req.data, 'data desde python')
        response = requests.patch(remote_url, json=req.data, params=req.GET, headers=headers)
    if req.method == 'GET':
